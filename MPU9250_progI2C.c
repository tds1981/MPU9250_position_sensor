...
uint8_t I2C_Byte_Read(uint8_t ACK)
{
	...
	for(int8_t i=7; i>=0; i--)
	{
		...
		if (SDA_status) res|= (1<<i);	
		Stop_Timer;
	}
		SCL_reset;
		Start_Timer;
		SDA_output;
		if (ACK) SDA_set; else SDA_reset;
		Pausa(Peiod/2);
		SCL_set;
		Pausa(Peiod);
		Stop_Timer;
...
uint8_t I2C_Byte_Write(uint8_t b)
{
	for(uint8_t i=0; i<8; i++)
	{
		...
		Pausa(Peiod/2);
		SCL_set;
		//if (SDA_status) return 127; // выход
		Pausa(Peiod);
		Stop_Timer;
		SDA_output;
		return 0;	
...
uint8_t I2C_Start(uint8_t direction) //0 - ведущий будет передавать информацию ведомому. 1 - ведущий будет получать информацию от ведомого
{
	SettingTimer;
	SCL_set; SDA_reset; //START condition	
	Start_Timer;
	Pausa(Peiod/2);
	Stop_Timer;
		
	if (I2C_Byte_Write(_address | direction)) return 127 ;	
	return 0;	
}
...
{
	I2C_Start(0);
	I2C_Byte_Write(WriteAddr);
	for (uint16_t i=0; i<NumByteToWrite; i++) I2C_Byte_Write(pBuffer[i]);
	I2C_Stop();
}
...
void MPU_Read(uint8_t *pBuffer, uint8_t ReadAddr, uint16_t NumByteToRead)
{
	...
	I2C_Start(1);
	for(uint8_t i=0; i<NumByteToRead-1; i++) pBuffer[i]=I2C_Byte_Read(0);
	pBuffer[NumByteToRead-1]=I2C_Byte_Read(1);
	I2C_Stop();
}
...
uint8_t MPU9250_Init()
{
	...
	DelayMiliSec(100); // long wait between AK8963 mode changes
